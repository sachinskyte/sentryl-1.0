...
const Admin: React.FC = () => {
  ...
  const fetchUsers = () => {
    try {
      setIsLoading(true);
      const raw = localStorage.getItem('sentryl_users');
      const storedUsers: Array<{ user: { id: string; email: string }; profile: { full_name: string | null; role?: string } }> = raw ? JSON.parse(raw) : [];
      
      // Always include built-in accounts
      const builtInUsers: User[] = [
        { id: 'admin-id', name: 'Admin User', email: 'admin@sentryl.com', role: 'admin', status: 'active' },
        { id: 'demo-id', name: 'Demo User', email: 'demo@example.com', role: 'user', status: 'active' },
      ];
      
      const registeredUsers: User[] = storedUsers.map(u => ({
        id: u.user.id,
      ...
      setUsers([...builtInUsers, ...registeredUsers]);
    } catch (error: any) {
