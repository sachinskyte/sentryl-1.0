...
import { getRecentReports, getReportsByUserId } from "@/lib/reportsStore";
...
const Dashboard: React.FC = () => {
  ...
  // Fetch recent reports from localStorage
  const { data: recentReports = [], isLoading: isLoadingReports } = useQuery({
    queryKey: ["recentReports"],
    queryFn: async () => {
      if (!user) return [];
      const data = getRecentReports(5);
  ...
  // Fetch user's personal reports count and bounties from localStorage
  const {
    data: userStats = { reportsCount: 0, totalBounty: 0 },
    isLoading: isLoadingUserStats,
  } = useQuery({
    queryKey: ["userStats", user?.id],
    queryFn: async () => {
      if (!user) return { reportsCount: 0, totalBounty: 0 };
      const userReports = getReportsByUserId(user.id);
      const totalBounty = userReports.reduce(
        (sum, report) => sum + (Number(report.bounty) || 0),
        0,
      );
      return { reportsCount: userReports.length, totalBounty };
    },
    enabled: !!user,
    refetchInterval: 5000,
  });
      ...
      {!isLoading && (
        <div className="cyber-panel p-4 mb-6">
          ...
          <p className="text-gray-300 mb-4">
            Your personal security dashboard is ready. Track vulnerabilities and
            ...
            <div className="p-4 bg-cyber-light/20 rounded-md border border-cyber-teal/20">
              <div className="flex justify-between items-center">
                <div>
                  <div className="flex items-center gap-1">
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p className="text-xs">
                            Bounties awarded for your verified reports
                          </p>
                        </TooltipContent>
                      </Tooltip>
                    </TooltipProvider>
                  ...
                  <p className="text-2xl font-bold text-white">
                    ${userStats.totalBounty.toLocaleString()}
                  </p>
                </div>
                <div className="h-8 w-8 flex items-center justify-center rounded-full bg-cyber-teal/20 text-cyber-teal font-bold">
                  $
                </div>
              </div>
            </div>
      ...
      <div className="cyber-panel p-4 border-cyber-teal/30">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-300">
          <div className="p-3 bg-cyber-light/30 rounded-md border border-cyber-teal/10">
            <h4 className="text-cyber-teal text-sm font-medium mb-1">
              Documentation Matters
            </h4>
            <p className="text-xs">
              Detailed vulnerability reports with clear steps to reproduce
              increase chances of verification and bounties.
            </p>
          </div>
          <div className="p-3 bg-cyber-light/30 rounded-md border border-cyber-teal/10">
            <h4 className="text-cyber-teal text-sm font-medium mb-1">
            ...
            <p className="text-xs">
              Always follow company disclosure policies and wait for fixes
              before publicly sharing details.
            </p>
          </div>
        </div>
      </div>
